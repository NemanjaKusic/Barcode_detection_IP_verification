`ifndef TEST_SLVERR_SV
`define TEST_SLVERR_SV 

// Inside Vivado: settings -> simmulation -> simulation add:
// -testplusarg UVM_TESTNAME=test_slverr -testplusarg UVM_VERBOSITY=UVM_LOW -sv_seed 2013

class test_slverr extends uvm_test;

    `uvm_component_utils(test_slverr)
	
	sobel_env env;
	memory_model shared_mem;
	axi_full_config axi_cfg;
	
	function new(string name = "test_slverr", uvm_component parent = null);
		super.new(name, parent);
    endfunction

    function void build_phase(uvm_phase phase);
		super.build_phase(phase);
		`uvm_info("TEST_SLVERR", "build_phase running", UVM_LOW)
		...
		axi_cfg.force_rresp = 2'b10;   // SLVERR
		axi_cfg.force_bresp = 2'b10;   // SLVERR
    ...
    task run_phase(uvm_phase phase);
        ...
        `uvm_info("TEST_SLVERR", "run_phase running", UVM_LOW)
        `uvm_info("TEST_SLVERR", $sformatf("%s", axi_cfg.print_mode()), UVM_LOW)
    
        base_addr = 32'h1000_0000;
        
        // ------ Preload input image from file ------
        `uvm_info("TEST_SLVERR", "Loading input image from file", UVM_LOW)
        ...
        `uvm_info("TEST_SLVERR", "Programming base_address register", UVM_LOW)
        ...
        // ------ Start pulse ------
        `uvm_info("TEST_SLVERR", "Issuing start pulse", UVM_LOW)
        start_seq = start_pulse_seq::type_id::create("start_seq");
        start_seq.start(env.lite_slave_agent.seqr);
    
        // ------ Wait for ready ------
        `uvm_info("TEST_SLVERR", "Waiting for DUT ready (timeout: 200 ms)", UVM_LOW)
        ...
        fork
            ...
            begin
                #200ms;
                `uvm_warning("TEST_SLVERR", "Timeout expired waiting for ready")
            end
        join_any
        disable fork;
    
        if (wait_seq.ready_observed) begin
            `uvm_info("TEST_SLVERR", $sformatf("DUT ready after %0d polls. Running scoreboard.",
                      wait_seq.reads_done), UVM_LOW)
            env.scoreboard.check_mem();
        end else begin
            `uvm_error("TEST_SLVERR", "DUT did not assert ready within timeout")
        end
