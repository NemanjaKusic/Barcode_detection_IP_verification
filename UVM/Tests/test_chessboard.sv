`ifndef TEST_CHESSBOARD_SV
`define TEST_CHESSBOARD_SV 

// Inside Vivado: settings -> simmulation -> simulation add:
// -testplusarg UVM_TESTNAME=test_chessboard -testplusarg UVM_VERBOSITY=UVM_LOW -sv_seed 2013

class test_chessboard extends uvm_test;

    `uvm_component_utils(test_chessboard)
	
	sobel_env env;
	memory_model shared_mem;
	axi_full_config axi_cfg;
	
	function new(string name = "test_chessboard", uvm_component parent = null);
		super.new(name, parent);
    endfunction

    function void build_phase(uvm_phase phase);
		super.build_phase(phase);
		`uvm_info("TEST_CHESSBOARD", "build_phase running", UVM_LOW)
    ...
    task run_phase(uvm_phase phase);
        ...
        `uvm_info("TEST_CHESSBOARD", "run_phase running", UVM_LOW)
        `uvm_info("TEST_CHESSBOARD", $sformatf("%s", axi_cfg.print_mode()), UVM_LOW)
    
        base_addr = 32'h1000_0000;
        
        // ------ Preload input image from file ------
        `uvm_info("TEST_CHESSBOARD", "Loading input image from file", UVM_LOW)
        // Put the golden file inside the root of Vivado generated project file 
        shared_mem.load_file(base_addr, "../../../../golden/IMG-09/input.txt");
    
        // ------ Tell scoreboard what to expect ------
        env.scoreboard.base_address   = base_addr;
        // Put the golden file inside the root of Vivado generated project file
        env.scoreboard.golden_gx_path = "../../../../golden/IMG-09/output_1.txt";
        env.scoreboard.golden_gy_path = "../../../../golden/IMG-09/output_2.txt";
		
		// ------ Program the base_address register ------
        `uvm_info("TEST_CHESSBOARD", "Programming base_address register", UVM_LOW)
        ...
        // ------ Start pulse ------
        `uvm_info("TEST_CHESSBOARD", "Issuing start pulse", UVM_LOW)
        start_seq = start_pulse_seq::type_id::create("start_seq");
        start_seq.start(env.lite_slave_agent.seqr);
    
        // ------ Wait for ready ------
        `uvm_info("TEST_CHESSBOARD", "Waiting for DUT ready (timeout: 200 ms)", UVM_LOW)
        ...
        fork
            ...
            begin
                #200ms;
                `uvm_warning("TEST_CHESSBOARD", "Timeout expired waiting for ready")
            end
        join_any
        disable fork;
    
        if (wait_seq.ready_observed) begin
            `uvm_info("TEST_CHESSBOARD", $sformatf("DUT ready after %0d polls. Running scoreboard.",
                      wait_seq.reads_done), UVM_LOW)
            env.scoreboard.check_mem();
        end else begin
            `uvm_error("TEST_CHESSBOARD", "DUT did not assert ready within timeout")
        end
