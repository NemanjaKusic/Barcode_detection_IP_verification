`ifndef TEST_DIRTY_DDR_SV
`define TEST_DIRTY_DDR_SV 

// Inside Vivado: settings -> simmulation -> simulation add:
// -testplusarg UVM_TESTNAME=test_dirty_ddr -testplusarg UVM_VERBOSITY=UVM_LOW -sv_seed 2013

class test_dirty_ddr extends uvm_test;

    `uvm_component_utils(test_dirty_ddr)
	
	sobel_env env;
	memory_model shared_mem;
	axi_full_config axi_cfg;
	
	function new(string name = "test_dirty_ddr", uvm_component parent = null);
		super.new(name, parent);
    endfunction

    function void build_phase(uvm_phase phase);
		super.build_phase(phase);
		`uvm_info("TEST_DIRTY_DDR", "build_phase running", UVM_LOW)
    ...
    task run_phase(uvm_phase phase);
        ...
        `uvm_info("TEST_DIRTY_DDR", "run_phase running", UVM_LOW)
        `uvm_info("TEST_DIRTY_DDR", $sformatf("%s", axi_cfg.print_mode()), UVM_LOW)
    
        base_addr = 32'h0000_1000;
		
		// ------ Preload the memory with non-zero(100) values around the zone where input and output pixels will be written ------
		`uvm_info("TEST_DIRTY_DDR", "Preloaded memory with non-zero values", UVM_LOW)
		shared_mem.fill(32'h0000_0000, 32'h0020_0000, 100);
        
        // ------ Preload input image from file ------
        `uvm_info("TEST_DIRTY_DDR", "Loading input image from file", UVM_LOW)
        ...
        `uvm_info("TEST_DIRTY_DDR", "Programming base_address register", UVM_LOW)
        ...
        // ------ Start pulse ------
        `uvm_info("TEST_DIRTY_DDR", "Issuing start pulse", UVM_LOW)
        start_seq = start_pulse_seq::type_id::create("start_seq");
        start_seq.start(env.lite_slave_agent.seqr);
    
        // ------ Wait for ready ------
        `uvm_info("TEST_DIRTY_DDR", "Waiting for DUT ready (timeout: 200 ms)", UVM_LOW)
        ...
        fork
            ...
            begin
                #200ms;
                `uvm_warning("TEST_DIRTY_DDR", "Timeout expired waiting for ready")
            end
        join_any
        disable fork;
    
        if (wait_seq.ready_observed) begin
            `uvm_info("TEST_DIRTY_DDR", $sformatf("DUT ready after %0d polls. Running scoreboard.",
                      wait_seq.reads_done), UVM_LOW)
            env.scoreboard.check_mem();
        end else begin
            `uvm_error("TEST_DIRTY_DDR", "DUT did not assert ready within timeout")
        end
