`ifndef TEST_START_WHILE_BUSY_SV
`define TEST_START_WHILE_BUSY_SV 

// Inside Vivado: settings -> simmulation -> simulation add:
// -testplusarg UVM_TESTNAME=test_start_while_busy -testplusarg UVM_VERBOSITY=UVM_LOW -sv_seed 2013

class test_start_while_busy extends uvm_test;

    `uvm_component_utils(test_start_while_busy)
	
	sobel_env env;
	memory_model shared_mem;
	axi_full_config axi_cfg;
	
	function new(string name = "test_start_while_busy", uvm_component parent = null);
		super.new(name, parent);
    endfunction

    function void build_phase(uvm_phase phase);
		super.build_phase(phase);
		`uvm_info("TEST_START_WHILE_BUSY", "build_phase running", UVM_LOW)
    ...
    task run_phase(uvm_phase phase);
        ...
        `uvm_info("TEST_START_WHILE_BUSY", "run_phase running", UVM_LOW)
        `uvm_info("TEST_START_WHILE_BUSY", $sformatf("%s", axi_cfg.print_mode()), UVM_LOW)
    
        base_addr = 32'h1000_0000;
		// Random offset
		offset = 32'h0100_0000;
		//offset = 100000;
        
        // ------ Preload input image from file ------
        `uvm_info("TEST_START_WHILE_BUSY", "Loading input image from file", UVM_LOW)
        ...
        `uvm_info("TEST_START_WHILE_BUSY", "Programming base_address register", UVM_LOW)
        ...
        // ------ Start pulse ------
        `uvm_info("TEST_START_WHILE_BUSY", "Issuing start pulse", UVM_LOW)
        start_seq = start_pulse_seq::type_id::create("start_seq");
        start_seq.start(env.lite_slave_agent.seqr);
    
        // ------ Wait for ready ------
        `uvm_info("TEST_START_WHILE_BUSY", "Waiting for DUT ready (timeout: 200 ms)", UVM_LOW)
        ...
        fork
            ...
            begin
                #70ms;
				// Starting IP during processing
				// The IP should not start the second time 
				
				// With the change of base_address there are errors in the scoreboard - shouldnt change it until ready = 1
				// ------ Program the base_address register ------
				`uvm_info("TEST_START_WHILE_BUSY", "Programming base_address register", UVM_LOW)
				pba_seq = program_base_addr_seq::type_id::create("pba_seq");
				...
				#150ms;
				`uvm_warning("TEST_START_WHILE_BUSY", "Timeout expired waiting for ready")
            end
        join_any
        disable fork;
    
        if (wait_seq.ready_observed) begin
            `uvm_info("TEST_START_WHILE_BUSY", $sformatf("DUT ready after %0d polls. Running scoreboard.",
                      wait_seq.reads_done), UVM_LOW)
            env.scoreboard.check_mem();
        end else begin
            `uvm_error("TEST_START_WHILE_BUSY", "DUT did not assert ready within timeout")
        end
