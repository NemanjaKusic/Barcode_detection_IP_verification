`ifndef TEST_REALISTIC_3_SV
`define TEST_REALISTIC_3_SV 

// Inside Vivado: settings -> simmulation -> simulation add:
// -testplusarg UVM_TESTNAME=test_realistic_3 -testplusarg UVM_VERBOSITY=UVM_LOW -sv_seed 2013

class test_realistic_3 extends uvm_test;

    `uvm_component_utils(test_realistic_3)
	
	sobel_env env;
	memory_model shared_mem;
	axi_full_config axi_cfg;
	
	function new(string name = "test_realistic_3", uvm_component parent = null);
		super.new(name, parent);
    endfunction

    function void build_phase(uvm_phase phase);
		super.build_phase(phase);
		`uvm_info("TEST_REALISTIC_3", "build_phase running", UVM_LOW)
    ...
    task run_phase(uvm_phase phase);
        ...
        `uvm_info("TEST_REALISTIC_3", "run_phase running", UVM_LOW)
        `uvm_info("TEST_REALISTIC_3", $sformatf("%s", axi_cfg.print_mode()), UVM_LOW)
    
        base_addr = 32'h1000_0000;
        
        // ------ Preload input image from file ------
        `uvm_info("TEST_REALISTIC_3", "Loading input image from file", UVM_LOW)
        // Put the golden file inside the root of Vivado generated project file 
        shared_mem.load_file(base_addr, "../../../../golden/IMG-14/input.txt");
    
        // ------ Tell scoreboard what to expect ------
        env.scoreboard.base_address   = base_addr;
        // Put the golden file inside the root of Vivado generated project file
        env.scoreboard.golden_gx_path = "../../../../golden/IMG-14/output_1.txt";
        env.scoreboard.golden_gy_path = "../../../../golden/IMG-14/output_2.txt";
		
		// ------ Program the base_address register ------
        `uvm_info("TEST_REALISTIC_3", "Programming base_address register", UVM_LOW)
        ...
        // ------ Start pulse ------
        `uvm_info("TEST_REALISTIC_3", "Issuing start pulse", UVM_LOW)
        start_seq = start_pulse_seq::type_id::create("start_seq");
        start_seq.start(env.lite_slave_agent.seqr);
    
        // ------ Wait for ready ------
        `uvm_info("TEST_REALISTIC_3", "Waiting for DUT ready (timeout: 200 ms)", UVM_LOW)
        ...
        fork
            ...
            begin
                #200ms;
                `uvm_warning("TEST_REALISTIC_3", "Timeout expired waiting for ready")
            end
        join_any
        disable fork;
    
        if (wait_seq.ready_observed) begin
            `uvm_info("TEST_REALISTIC_3", $sformatf("DUT ready after %0d polls. Running scoreboard.",
                      wait_seq.reads_done), UVM_LOW)
            env.scoreboard.check_mem();
        end else begin
            `uvm_error("TEST_REALISTIC_3", "DUT did not assert ready within timeout")
        end
